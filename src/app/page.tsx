	...
	({
			...
			return allProducts
				.filter(
					(name) =>
						...
						!excludedProducts.includes(name)
				)
				.slice(0, 8);
		}, [inputValue, allProducts, excludedProducts]);
		...
		const handleInputKeyDown = (
			...
			if (e.key === "Enter" && inputValue.trim()) {
				...
				if (exactMatch && !excludedProducts.includes(exactMatch)) {
		...
		return (
			<div className="space-y-3">
				<div className="relative">
					<input
						type="text"
						placeholder="Type to search and add products to exclude..."
						value={inputValue}
